    ...
    with service.SessionFactory() as session:
        row = session.scalars(
            ...
            .where(
                ...
                TaskFileMutation.before_snapshot_id.is_not(None),
            )
        ...
        file_snapshot = session.get(FileSnapshot, row.before_snapshot_id)
        if file_snapshot is None or not file_snapshot.snapshot_ref:
            return _unavailable(relative_path, "previous_snapshot_unavailable")
        snapshot = session.get(SnapshotSet, file_snapshot.snapshot_set_id)
        if snapshot is None:
            return _unavailable(relative_path, "previous_snapshot_unavailable")

        snapshot_path = (Path(snapshot.snapshot_root) / file_snapshot.snapshot_ref).resolve()
        ...
        baseline = {
            ...
            "snapshot_ref": file_snapshot.snapshot_ref,
        }
