...
def get_system_database_path(db_name: str, system_data_root: str = None) -> str:
    """Get the full path for a system database file.

    Args:
        db_name: Name of the database file (without .db extension)
        system_data_root: Optional override for system data directory
    ...
    os.makedirs(system_data_root, exist_ok=True)
    return os.path.join(system_data_root, f"{db_name}.db")


def create_engine_from_system_db(db_name: str):
    """Create SQLAlchemy engine for a system database with runtime context support.

    Automatically uses the system_data_root from runtime context when available,
    ...
    if has_runtime_context():
        ...
        system_data_root = str(runtime.config.system_data_root)
        database_path = get_system_database_path(db_name, system_data_root)
    else:
