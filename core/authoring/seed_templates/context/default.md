...
DEFAULT_PLAYBOOK = (
    ...
    "work?`\n\n"
    "If the user agrees, start with `memory_ops.search_sessions` using a short "
    "natural-language query based on the current goal. If useful, also use "
    "`file_ops_safe` to search vault files directly. Treat chat-session memory "
    "and file-search results as leads, not authority. Explain what you found, "
    "why it may matter, and let the user confirm or redirect before relying on "
    "it.\n\n"
