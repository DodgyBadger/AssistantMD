...
schedule: "cron: 0 2 * * *"
enabled: false
description: Extract session memory for chat sessions that do not yet have derived memory.
---
...
```python
"""Extract memory for a bounded batch of chat sessions pending memory."""
...
pending = await retrieve_sessions(selection="pending_memory", limit=BATCH_SIZE)
sessions = list(pending.items)

if not sessions:
    await finish(status="skipped", reason="no sessions pending memory extraction")
