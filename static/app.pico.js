...
function setupTabs() {
    Object.entries(tabs).forEach(([name, tabControls]) => {
        if (tabControls.button) {
            tabControls.button.addEventListener('click', (event) => {
...
function switchTab(tabName) {
    Object.entries(tabs).forEach(([name, tabControls]) => {
        ...
        tabControls.button.setAttribute('aria-current', isActive ? 'page' : 'false');
        tabControls.content.hidden = !isActive;
    });
...
function populateSelectors() {
    ...
    // Append tools in priority order
    appendTools(leftColumnOrder);
    appendTools(rightColumnOrder);
...
async function fetchSystemStatus() {
    try {
        ...
        dashElements.systemStatus.innerHTML = '<p class="text-error">Failed to fetch system status</p>';
    }
}
...
function displaySystemStatus() {
    ...
    if (status.scheduler.job_details && status.scheduler.job_details.length > 0) {
        ...
        html += `
            <h3>⏰ Scheduled Jobs ${schedulerBadge}</h3>
            <table class="dashboard-table">
                <thead>
                    <tr>
                        <th>Workflow</th>
...
function addLoadingMessage() {
    ...
    contentDiv.className = 'message-card message-bubble';
    contentDiv.innerHTML = `<div class="stream-progress">
        <span class="stream-status-indicator typing">${TYPING_DOTS_HTML}</span>
        <span class="stream-status-text">Contacting assistant…</span>
    </div>`;

    messageDiv.appendChild(contentDiv);

    if (chatElements.chatMessages.children.length === 1 &&
        chatElements.chatMessages.children[0].classList.contains('empty-state')) {
...
function createAssistantStreamingMessage() {
    ...
    toolList.className = 'tool-status-list';
    toolList.hidden = true;

    const bodyDiv = document.createElement('div');
    bodyDiv.className = 'message-body';
    bodyDiv.innerHTML = '<p class="text-small text-muted">Preparing response…</p>';
...
function renderAssistantMarkdown(context) {
    if (!context.fullText.trim()) {
        context.bodyDiv.innerHTML = '<p class="text-small text-muted">Thinking…</p>';
    } else {
...
async function clearSession() {
    ...
    chatElements.chatMessages.innerHTML = '<div class="empty-state">Start a conversation...</div>';
    updateStatus();
}
...
async function rescanVaults() {
    ...
    dashElements.rescanResult.innerHTML = '<p class="text-muted">Rescanning...</p>';
    ...
    try {
        ...
        dashElements.rescanResult.innerHTML = `
            <div class="banner success">
                <p><strong>✅ Rescan Completed</strong></p>
                ...
                <p class="text-small text-muted">${data.message || ''}</p>
            </div>
        `;
        ...
        dashElements.rescanResult.innerHTML = `<p class="text-error">❌ Error: ${error.message}</p>`;
    } finally {
...
async function executeWorkflow() {
    ...
    dashElements.executeWorkflowResult.innerHTML = '<p class="text-muted">Executing...</p>';
    ...
    try {
        ...
        dashElements.executeWorkflowResult.innerHTML = `
            <div class="banner success">
                ...
                ${outputFiles.length ? `
                    <p>Output files created:</p>
                    <ul>
                        ${outputFiles.map(f => `<li>${f}</li>`).join('')}
                    </ul>
                ` : ''}
                <p class="text-small text-muted">${data.message || ''}</p>
            </div>
        ...
        dashElements.executeWorkflowResult.innerHTML = `<p class="text-error">❌ Error: ${error.message}</p>`;
    } finally {
...
function updateStatus(message) {
    ...
    if (!noticeLines.length) {
        ...
        chatElements.statusText.hidden = true;
        chatElements.statusText.innerHTML = '';
    } else {
        chatElements.statusIcon.textContent = '⚠️';
        chatElements.statusText.hidden = false;
        chatElements.statusText.textContent = noticeLines.join(' • ');
    }
}
