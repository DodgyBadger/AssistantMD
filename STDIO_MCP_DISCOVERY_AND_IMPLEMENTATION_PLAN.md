...
The test response keeps the existing readiness shape and adds advertised MCP
capability names. `ready` means the fixed executable launched, MCP initialized,
...
5. captures only bounded stderr diagnostics without placing them in model
   context or allowing log flooding;
6. advertises configured Roots and calls only server capabilities advertised at
   initialization; and
...
advertised capabilities and explain the bounded meaning of readiness.
...
- `mcp_connection_ready`: connection ID, principal ID, transport, tool count,
  advertised capability names; URL only for HTTP/SSE, never launch arguments or
  environment values;
- `mcp_companion_stdio_rejected`: connection ID when available and stable reason
  (`restricted_mode`, `invalid_launch`, or `companion_unavailable`);
...
1. Define the smallest persisted connection/API/UI contract from the completed
   experiments, omitting skill paths and installation commands.
2. Add validation-first assertions for advanced-mode gating, ownership,
   structured launch validation, capability-aware testing, frozen catalogs, and
   retained process cleanup.
3. Implement the companion stdio connection type behind advanced mode.
4. Treat reviewed skill import/adaptation as a separate future feature.
