...
- TTL syntax: support s/m/h/d (e.g.`@cache 10m`, `@cache 24h`, `@cache 1d`, `@cache 30s`), `daily` (rolls over if called new day), `weekly` (rolls over if called new week, per week_start_day), `session` (only valid for current chat session).
...
- **Storage**: store outputs per step in `system/cache.db` via a cache store helper (new module or extension of existing store).
...
1. **Add directive** ✅
   ...
   - Parse duration with suffix s/m/h/d plus `daily`, `weekly`, `session`; validate value.
