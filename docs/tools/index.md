...
- use `file_ops_safe(operation="list", target="__virtual_docs__/tools")` to inspect the tool docs directory
- use `file_ops_safe(operation="search", target="tool-name-or-concept", scope="__virtual_docs__/tools")` to find the right doc or section
- use `file_ops_safe(operation="read", target="__virtual_docs__/tools/<tool_name>")` to read the matching tool doc
