...
The validation harness discovers the application root from its checkout, so the
CLI works from a checkout-local UV environment regardless of where the checkout
is mounted. Set `VALIDATION_APP_ROOT` to override the checkout root used for
fixtures and default runtime paths. Set `VALIDATION_ROOT` when scenario
