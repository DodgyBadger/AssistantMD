...
- Changing a Google OAuth client ID invalidates its client secret, pending authorization, connected account, granted scopes, and capability readiness. Display-name, default-selection, and Gmail preference changes preserve authorization. Replacing a client secret preserves the connection metadata but requires a new authorization grant.
- Google browser authorization returns through one stable provider callback. Cryptographic state resolves the initiating named connection; connection item routes retain start, manual completion, and disconnect operations without defining separate browser callback identities.
...
- Google and MCP browser authorization polling has one owner per connection. Restarting an attempt supersedes its prior request, and configuration mutations, tab deactivation, and page teardown cancel active polling so stale responses cannot rerender cards or replace user feedback.
